...
\def\currentChapter{Graph clustering: spectral clustering and the Stochastic Bloc Model}
...
\begin{frame}
  ...
  \begin{block}{now: \alert{clustering of network based on a probabilistic model of the graph}}<2->
