...
<<sbm, eval = FALSE, echo = FALSE, cache = TRUE>>=
my_sbm <- 
...
<<fblog , cache = TRUE, results='hide', echo = FALSE, message=FALSE>>=
library(blockmodels)
  mySBM_collection <- BM_bernoulli(
   "SBM_sym", 
   blog %>% as_adj(sparse = FALSE), verbosity = 0, 
