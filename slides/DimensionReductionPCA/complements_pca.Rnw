...
fviz_pca_ind(crabs_pca_corrected,
  col.ind = paste(crabs_corrected$species, crabs_corrected$sex), palette = pal)
@
\end{frame}
...
fviz_pca_biplot(crabs_pca_corrected,
  col.ind = paste(crabs_corrected$species, crabs_corrected$sex), palette = pal)
@
\end{frame}
...
<<>>=
crabs_pca_corrected <- crabs_corrected %>% 
  FactoMineR::PCA(graph = FALSE, quanti.sup = 7, quali.sup = c(1,2))
@
\end{frame}
...
<<>>=
fviz_pca_ind(crabs_pca_corrected, habillage = "species", col.ind.sup = "black", addEllipses = TRUE)
@
