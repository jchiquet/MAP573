...
\begin{frame}
  ...
  \begin{block}{Criteria: intra-class variance/ Within-inertia}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item[$\rightsquigarrow$] defined the indexed hierarchy (height of the dendrogram)
    \item[$\rightsquigarrow$] Same criteria as in K-means algorithm
  \end{itemize}
  }
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hlstd{(}\hlkwd{rev}\hlstd{(Ward}\hlopt{$}\hlstd{height),} \hlkwc{xlab} \hlstd{=} \hlstr{"number of clusters"}\hlstd{,} \hlkwc{ylab} \hlstd{=} \hlstr{"height"}\hlstd{)}
\end{alltt}
\end{kframe}
